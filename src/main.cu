...
int main() {
  ...
  Simulation
      *simulation; // = static_cast<Simulation*>(malloc(sizeof(Simulation)));
  ...
  int sampleEvery = ((static_cast<int>(1 / simulation->timestepSize)) /
                     simulation->outputFrameRate);
