  ...
  for (int i = 0; i < simulation->numParticles; i += 1) {
    for (int j = i + 1; j < simulation->numParticles; j++) {
      ...
      double distanceSquared =
          pow(particleData->positionX[i] - particleData->positionX[j], 2) +
      ...
      if (distanceSquared < 4) {
        ...
        // Unforutnately AVX code is unreadable so this may be difficult to
        // follow.

        // Load particle coordinates into a vector
        __m256d positionVector = _mm256_set_pd(
            particleData->positionY[i], particleData->positionY[j],
            particleData->positionX[i], particleData->positionX[j]);
        // fill a vector with the distances
        double distance = sqrt(distanceSquared);
        __m256d distanceVector = _mm256_set1_pd(distance);
        // Divide positions by distance
        __m256d positionOverDistance =
            _mm256_div_pd(positionVector, distanceVector);
        // Rearrange positionOverDistance such that the differences between the
        // I and J values may be taken
        __m256d shiftedPositionDistance =
            _mm256_permute4x64_pd(positionOverDistance, 0b10110001);
        __m256d deltaPositionOverDistance =
            _mm256_sub_pd(positionOverDistance, shiftedPositionDistance);
        ...
        // in only two AVX mul_pd and one hadd_pd we can calculate 4 dot
        // products!

        // load x and y components of each velocity into vector
        __m256d velocityVector = _mm256_set_pd(
            particleData->velocityX[i], particleData->velocityY[i],
            particleData->velocityX[j], particleData->velocityY[j]);
        ...
        // Now, since the vectors being dotted are 2x1, we can use hadd for the
        // summing step
        ...
        double newPositionXI =
            particleData->positionX[i] -
            overlap *
                (particleData->positionX[i] - particleData->positionX[j]) /
                distance;
        double newPositionYI =
            particleData->positionY[i] -
            overlap *
                (particleData->positionY[i] - particleData->positionY[j]) /
                distance;
        double newPositionXJ =
            particleData->positionX[j] +
            overlap *
                (particleData->positionX[i] - particleData->positionX[j]) /
                distance;
        double newPositionYJ =
            particleData->positionY[j] +
            overlap *
                (particleData->positionY[i] - particleData->positionY[j]) /
                distance;
